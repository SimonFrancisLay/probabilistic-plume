...
def step_once_batched(T: np.ndarray, params: SimParams, rng: np.random.Generator) -> Tuple[np.ndarray, Dict[str, float]]:
    ...
    # Pre compute neighbor indices for the centre cell only, since all parcels are injected there
    i, j = c, c
    nbrs = {
    ...
    # Movement probabilities computed from time t
    ...
    # Vertical centroid of heat: y index increases upward if we set origin lower
    # Our imshow uses origin="lower", so row 0 is bottom. Use that convention here.
    y = np.arange(N, dtype=np.float64)
    # Broadcast y over columns
